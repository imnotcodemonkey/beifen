...
function GetDDImage($litpic, $picname, $isremote)
{
     ...
     !empty($_FILES[$litpic]['tmp_name']) && is_uploaded_file($_FILES[$litpic]['tmp_name']))
    {
        ...
        CreateDir($savepath);
        $fullUrl = $savepath.'/'.MyDate('m-d_His',time()).'-'.$cuserLogin->getUserID().'-'.mt_rand(10, 99);//leihaobin 2011-12-07 19:48:38 //dd2char(MyDate('mdHis', $ntime).$cuserLogin->getUserID().mt_rand(1000, 9999));
