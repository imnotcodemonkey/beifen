...
if ( ! function_exists('GetFileName'))
{
    function GetFileName($aid,$typeid,$timetag,$title,$ismake=0,$rank=0,$namerule='',$typedir='',$money=0,$filename='')
    {
        ...
        else
        {
            ...
            $arr_rpvalues = array($articleDir,$y, $m, $d, $timetag, $aid, dd2char($d.$m.$aid));//leihaobin
            if($filename != '')
            {
                if(strpos('/',$filename)>=0){ //leihaobin 用于将文件生成在上级目录之后修改链接路径 2012年3月18日 01:08:34
                	$articleRule = str_replace('..','', str_replace('../..','',$filename)).$GLOBALS['cfg_df_ext'];
                	$articleRule = str_replace('index.html','',$articleRule);
                }
                else
...
if ( ! function_exists('GetTypeUrl'))
{
    function GetTypeUrl($typeid,$typedir,$isdefault,$defaultname,$ispart,$namerule2,$moresite=0,$siteurl='',$sitepath='')
    {
        ...
        if($isdefault==-1)
        {
            ...
            $reurl = "/c/list-".$typeid.".html";// leihaobin 2012.04.10
        }
        else if($ispart==2)
        {
            //跳转网址
