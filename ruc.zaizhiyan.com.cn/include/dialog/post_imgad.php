...
if(!empty($noeditor)){
	...
	ShowMsg("成功上传一幅图片！","select_imgad.php?imgstick=$imgstick&comeback=".urlencode($filename_name)."&v=$v&f=$f&CKEditorFuncNum=$CKEditorFuncNum&noeditor=yes&dir=".urlencode($dir)."&d=".time());
}else{
	ShowMsg("成功上传一幅图片！","select_imgad.php?imgstick=$imgstick&comeback=".urlencode($filename_name)."&v=$v&f=$f&CKEditorFuncNum=$CKEditorFuncNum&dir=".urlencode($dir)."&d=".time());
}
exit();