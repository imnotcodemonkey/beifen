...
if(!is_dir($cfg_basedir.$dir."/$mdir"))
{
    MkdirAll($cfg_basedir.$dir."/$mdir",$cfg_dir_purview);
    CloseFtp();
}
...
else
{
    if(in_array($imgfile_type, $cfg_photo_typenames))
    {
		...
		//如果图片超过规定宽度自动缩放 leihaobin 2011-12-08 00:29:35
		if(isset($cfg_img_maxwith)){
			if($imgwidthValue>$cfg_img_maxwith){
				$newHeight=floor($imgwidthValue/$cfg_img_maxwith*$imgheightValue);
				ImageResize($fullfilename, $cfg_img_maxwith, $newHeight);
				$imgwidthValue = $cfg_img_maxwith;
				$imgheightValue = $newHeight;
			}
		}
...
if(!empty($noeditor)){
	...
	ShowMsg("成功上传一幅图片！","browse_images.php?imgstick=$imgstick&comeback=".urlencode($filename_name)."&v=$v&f=$f&CKEditorFuncNum=$CKEditorFuncNum&noeditor=yes&dir=".urlencode($dir)."/$mdir&d=".time());
}else{
	ShowMsg("成功上传一幅图片！","browse_images.php?imgstick=$imgstick&comeback=".urlencode($filename_name)."&v=$v&f=$f&CKEditorFuncNum=$CKEditorFuncNum&dir=".urlencode($dir)."/$mdir&d=".time());
}
exit();