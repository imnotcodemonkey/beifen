	...
	return {
		...
		onLoad :  function()
		{
			var columns = this.definition.charColumns,
				...
				chars = CKEDITOR.config.specialChars;
	chars = [
		'&copy;', '※', '＄', '￡', '￥', '￠', '€', '⊙', '◎', '●', '○', '¤', '㊣', '■', '□', '★', '☆', '◆', '◇', '▲', '△', '▼', '▽',
		'「', '」', '『', '』', '〖', '〗', '【', '】', '↑', '↓', '→', '←', '↘', '↙', '♀', '♂', '┇', '┅', '‖', '※',
		'卍', '卐', '∞', '∥', '∠', '≌', '∽', '≦', '≧', '≒', '﹤', '﹥', '≈', '≡', '≠', '＝', '≤', '≥', '＜', '＞', '≮', '≯',
		...
		'ā', 'á', 'ǎ', 'à', 'ō', 'ó', 'ǒ', 'ò', 'ē', 'é', 'ě', 'è', 'ī', 'í', 'ǐ', 'ì', 'ū', 'ú', 'ǔ', 'ù', 'ǖ', 'ǘ', 'ǚ', 'ǜ', 'ü', 'ê', 'ń', 'ň',
		'═', '║', '╒', '╓', '╔', '╕', '╖', '╗', '╘', '╙', '╚', '╛', '╜', '╝', '╞', '╟', '╠', '╡', '╢', '╣', 
		'╤', '╥', '╦', '╧', '╨', '╩', '╪', '╫', '╬', '╳', '╔', '╗', '╝', '╚', '╬', '═', '╓', '╩', '┠', '┨', 
		'┯', '┷', '┏', '┓', '┗', '┛', '┳', '⊥', '﹃', '﹄', '┌', '╭', '╮', '╯', '╰', '─', '━', '│', '┃', 
		'┄', '┅', '┆', '┇', '┈', '┉', '┊', '┋', 
			...
			while ( i < size )
			{
				...
				for ( var j = 0 ; j < columns ; j++, i++ )
				{
					if ( ( character = chars[ i ] ) )
					{
						...
						html.push(
							'<td class="cke_dark_background" style="cursor: default; width:16px;" role="presentation">' +
							...
							' tabindex="-1">' +
							'<span style="margin: 0 auto;cursor: inherit;font:14px/100% Simsun;">' +
							character +
							'</span>' +
							'<span class="cke_voice_label" id="' + charLabelId + '" style="font:14px/100% Simsun;">' +
							charDesc +
							'</span></a>');
					}
					else
