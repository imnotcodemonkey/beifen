...
class DataListCP
{
    ...
    function GetPageList($atts,$refObj='',$fields=array())
    {
        ...
        $ajaxFun='';//leihaobin 2012年3月24日 00:32:15
        if(count($this->getValues)>0)
        {
            foreach($this->getValues as $key=>$value)
            {
                ...
                if($key=='ajaxFun'){$ajaxFun=$value;}//leihaobin 2012年3月24日 00:32:15
            }
        }
        $purl .= "?".$geturl;
        ...
        $plist .= "</div>\n";
        if($ajaxFun!=''){
        	$plist=str_replace('href="','href="#'.$ajaxFun.'" onclick="'.$ajaxFun.'(\'',$plist);
