...
class DedeVote
{
    ...
    function GetVoteResult($tablewidth="600", $lineheight="24", $tablesplit="40%")
    {
        ...
        {
            $totalcount=1;
        }//2012-03-31 18:24:20 leihaobin
        $res = "<table width='$tablewidth' border='0' cellspacing='0' cellpadding='0'>\r\n";
        $res .= '<thead><tr class="TC"><td id="v1">序号</td><td id="v2">选项</td><td id="v3">比例</td><td id="v4">人数</td></tr></thead>'."\r\n";
        $i=1;
        foreach($this->VoteNotes as $k=>$arr)
        {
            $res .= "<tr><td class=TC>".$i.".</td>";
            $res .= "<td>".$arr['name']."</td>";
            
              $c = $arr['count'];
              $res .= "<td><span><img src='/res/2012/voteBg.png' style='width:".round(($c/$totalcount)*100,2)."%;height:9px'></span><tt>".round(($c/$totalcount)*100,2)."%</tt></tt></td><td class='TR'>".$c."</td>";
            $res .= "</td></tr>\r\n";
            $i++;
        }
        $res .= "</table>\r\n";
