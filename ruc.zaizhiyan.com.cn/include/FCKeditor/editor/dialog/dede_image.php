...
if($dopost=='upload')
{
	...
	for($i=1; $i <= $totalform; $i++)
	{
		...
		$filename = MyDate('Ymd_His',$nowtme).'-'.$i.'_'.$cuserLogin->getUserID();
	 ...
	 if($cfg_remote_site=='Y' && $remoteuploads == 1)
	 {
	 	  ...
	 	  $imgHtml .=  "<img src=\"$imgsrcValue\" width=\"$imgwidthValue\" border=\"0\" height=\"$imgheightValue\" alt=\"$altname\" /><br />\r\n";
	 } else {
			if($cfg_multi_site=='N')
			{
				$imgHtml .=  "<img src=\"$imgsrcValue\" width=\"$imgwidthValue\" border=\"0\" height=\"$imgheightValue\" /><br />\r\n";
			}
			else
			{
